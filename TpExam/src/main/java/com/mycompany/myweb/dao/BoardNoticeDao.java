...
public class BoardNoticeDao {
	...
	public BoardNotice search(String btitle, String bcontent, int pageNo, int rowsPerPage){
		String sql = "select btitle from board_Notice where bno in ((select max(bno) from board_Notice where bno < ?) ";
		List<BoardNotice> list = jdbcTemplate.query(
			sql, 
			new Object[] {btitle,bcontent,(pageNo*rowsPerPage), ((pageNo-1)*rowsPerPage + 1)}, 
