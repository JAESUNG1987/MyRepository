...
select dname 
from dept
where not deptno in (select deptno from emp );
...
--emp 테이블 복제해서 emp2로 만드세요(하나의 DDL 로 작성하시오)
...
--제약조건 과 인덱스는 복사하지 않아도 됨.
create table emp3
as 
select * from emp where 1=0;

--3.3 서브 쿼리로 데이터를 삽입하기 
--emp3을 만들었다면 10번 부서에 사원들만 복사하세요.
insert into emp3 
select * from emp where deptno=10;

--3.4 서브 쿼리를 이용한 데이타 변경
