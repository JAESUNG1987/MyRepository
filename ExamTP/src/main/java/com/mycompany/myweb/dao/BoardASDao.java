...
import com.mycompany.myweb.controller.BoardAsController;
...
public class BoardASDao {
	...
	public List<BoardAS> selectByPage(int pageNo, int rowsPerPage) {
		...
		sql += "from (select bano, batitle, bacontent, bawriter, badate from board_AS order by bano desc) ";
		sql += "where rownum<=? ";
		sql += ") ";
		sql += "where rn>=? ";
